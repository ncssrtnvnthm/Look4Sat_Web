...
private fun getOrCreateObject(jsonOrbitalData: String): OrbitalObject? {
    return try {
        ...
        println("JsBridge: failed to parse OrbitalData: ${e.message}")
        null
    }
}
 ...
 */
@JsExport
fun look4satGetSunTimes(lat: Double, lon: Double, timeMs: Double): String {
    val pos = GeoPos(lat, lon, 0.0)
    val times = CelestialComputer.findSunRiseSet(pos, timeMs.toLong())
